package cn.wjx.spring_security_demo.auth.filters;

import cn.wjx.spring_security_demo.auth.utils.*;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (redisUtil.isBlackList(authToken)) {
                ...
                response.getWriter().write(JSON.toJSONString(JZBResult.build(30015,"token失效")));
                return;
            }
            ...
            if (redisUtil.hasKey(authToken)) {//判断redis是否有保存
                ...
                if (JwtTokenUtil.isExpiration(expirationTime)) {
                    ...
                    if (DateUtil.compareDate(currentTime, tokenValidTime)) {
                        ...
                        response.getWriter().write(JSON.toJSONString(JZBResult.build(30017,"过期TOKEN，不予刷新")));
                        return;
            ...
            if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (!StringUtil.equals(ip, currentIp)) {//地址不正确
                    ...
                    if (redisUtil.isBlackList(authToken)) {
                        log.info("用户：{}的token：{}在黑名单之中，拒绝访问",username,authToken);
                        response.getWriter().write(JSON.toJSONString(JZBResult.build(30019,"此token位于黑名单")));
                        return;
                    }
                    //黑名单没有则继续，如果黑名单存在就退出后面
                }

                UserInfo userDetails = userService.getUserInfo(username);
